...
class Map extends React.Component {
    ...
    slideToCorrectImage = () => {
        if (this.state.currentSlide === 0) {
            document.getElementsByClassName('slide-container')[0].style.transform = `translateX(0)`;
        } else if (this.state.currentSlide === 1) {
            document.getElementsByClassName('slide-container')[0].style.transform = `translateX(calc(-100% + 100px))`;
        } else {
            document.getElementsByClassName('slide-container')[0].style.transform = `translateX(calc(${-100 * (this.state.currentSlide)}% + ${100 + 80 * (this.state.currentSlide - 1)}px))`;
        }
    };

    checkDirection = () => {
        if (touchEndX < touchStartX - window.innerWidth / 8) {
            console.log('right', this.state.currentSlide);
                            ...
                            this.props.spots.map((spot, i) => {
                                return (
                                    <div className="slide">
                                        <img
                                            loading="lazy"
                                            src={`${config.s3baseUrl}${this.props.spots[i].images[0]}`}
                                            onTouchStart={this.handleTouchStart}
                                            onTouchEnd={this.handleTouchEnd}
                                        />
                                        <div className="slide-title">{this.props.spots[i].title.split('-')[0].trim()}</div>
                                    </div>
                                );
                            })
                        }
